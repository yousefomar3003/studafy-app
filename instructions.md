...
| Why necessary | AI is the only capability with a shipped user interface and no task behind it. Four student screens (`student_ai_page`, `student_ai_ask_page`, `student_ai_practice`, `student_ai_insights`) and the `study_coach` feature slice exist today. `propose-paper-grade` is hard-disabled by SEC-001, but `study-coach` is not: it forwards lesson material to whatever `STUDY_COACH_URL` names, gated only by an environment variable, with credentials absent from §22. Leaving this unresolved ships either a visibly dead feature (Apple 2.1 non-functional) or an unreviewed third-party egress path for minors' school data. |
| Existing files affected | `supabase/functions/study-coach`, `supabase/functions/propose-paper-grade`, `lib/features/study_coach/**`, the four student AI pages, `lib/core/runtime_environment.dart:31`, `supabase/functions/.env.example`. |
| New modules/files expected | If enabling: an AI Hono module with an egress allowlist, prompt/response redaction, per-tenant quotas and an audit trail. If removing: deletion of the AI surface and its Edge Functions, plus a migration retiring `ai_grading_drafts`/`question_suggestions` reads. |
| Dependencies | FILE-051 clean immutable file objects; AUTH-031 relationship authorization; API-041 grading transactions; **a signed DPA and a DPIA extended to cover AI processing of minors' data**. The legal gate is independent of every engineering phase and may never clear. |
| Detailed tasks | Decide and record the direction with a named owner. **Enable path:** replace the generic forward-to-a-URL pattern with a named provider adapter; accept only server-owned `file_object_id`; enforce tenant/relationship authorization per request; redact student identifiers before egress; add an egress allowlist so a compromised variable cannot retarget the provider; quotas and cost caps; retain prompts/responses under the §15 retention schedule; teacher review before any AI proposal affects a grade. **Removal path:** delete both Edge Functions, remove the AI screens and the `study_coach` slice, remove `STUDY_COACH_*`, and leave no disabled-looking surface for a reviewer to find. |
| Database changes | Enable path: AI request/response audit with tenant scope and retention. Removal path: forward migration retiring the unused AI relations. |
| API changes | Enable path: `/v1` AI endpoints replacing both Edge Functions. Removal path: none; the routes cease to exist. |
| Security requirements | No caller-supplied path or URL ever reaches egress; no environment variable re-enables AI without a reviewed code change; service-role credentials never sign an object for a provider; provider egress restricted to an allowlisted host; no student name, email, or submission content leaves the tenant unredacted. |
| Tests required | Path/URL substitution denied; cross-tenant material denied; egress-allowlist bypass denied; redaction proven on real fixture shapes; quota exhaustion; provider outage and timeout; and — on the removal path — a test asserting no AI route, screen, or credential remains. |
| Observability | Egress volume and destination, redaction failures, quota denials, provider latency and error rate, cost per tenant. |
| Risks | Unlawful processing of minors' data; provider retaining prompts for training; cost runaway; a reviewer finding a dead feature; AI output treated as an authoritative grade. |
| Rollback | The capability stays behind a reviewed guard, not a flag. Reverting means restoring the guard and the ADR that removed it. |
| Deliverables | An ADR recording the direction and its evidence, the DPA and DPIA references or their explicit absence, and either the reviewed AI module or proof of complete removal. |
| Acceptance criteria | The shipped app contains no AI surface that cannot lawfully function. Either every AI screen works against a reviewed, authorized, redacted pipeline, or no AI screen exists. |
| Complexity | L (removal) to XL (enable) |
| Parallel | The decision and the legal work can proceed alongside Phases 5–7; the engineering cannot start until the direction is decided. |
| Conditions before proceeding | A named owner for the AI decision; for the enable path, a signed DPA and an extended DPIA. Without both, the removal path is the only lawful outcome. |


**Phase 7 gate:** mobile core is server authoritative and offline-safe; native identity/signing/privacy/accessibility ready; store transactions are verified, idempotent and lifecycle-complete; the AI decision is resolved so no unusable AI surface ships; old supported clients remain safe.

-- Teachers create classes (MOB/API-041 correction).
--
-- Replaces private.api041_command so 'createClassroom' accepts an active
-- teacher, not only a school administrator, and records the lead teacher
-- against a teaching membership. Forward-only: the function is replaced in
-- full because INFRA-081 forbids editing a shipped migration.
...
-- ---------------------------------------------------------------------------
--
-- private.authz_authorize runs before this function is ever reached, and its
-- classroom.create branch asks is_school_admin. Relaxing api041_command alone
-- therefore changed nothing a caller could observe: the request was refused
-- one layer earlier, and a denial on a concealed resource is reported as 404,
-- which is what a teacher saw as "this record no longer exists".
--
-- Chained the way FILE-051 chained its predecessor: the previous generation is
-- preserved under a new name and consulted for every other action.

create or replace function private.authz_authorize_pre_teacher_class(

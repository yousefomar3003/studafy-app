-- Lesson sessions report whether their content has been filed (TEACH-055).
--
-- The teacher's screen has to separate sections still owing content from
-- those already closed, and status cannot say: recordAttendance leaves a
-- session 'completed' too. Forward-only, so the query surface is replaced
-- in full.
...
begin
  ...
  case p_operation
    ...
    when 'listLessonSessions' then
      select coalesce(jsonb_agg(to_jsonb(x) order by x.id),'[]'),max(x.id::text) into result,next_pos from (
        select ls.id,ls.classroom_id "classroomId",ls.starts_at "startsAt",ls.ends_at "endsAt",ls.title,ls.status,ls.filed_at "filedAt",ls.version

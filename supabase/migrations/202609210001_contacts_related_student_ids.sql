-- Contacts carry the student ids they relate to (ROSTER-054).
--
-- V1Contact already told staff which children a guardian belongs to, but by
-- display name only. A teacher's roster maps a child to their guardians, and
-- two children in one class can share a name, so matching by name would
-- attach one family's contact to another child's record.
--
-- Forward-only: the function is replaced in full because INFRA-081 forbids
-- editing a shipped migration. Its place in the api042_query chain is
-- unchanged; only the contacts projection gains a field.

CREATE OR REPLACE FUNCTION private.api042_query_pre_guardian_reads(p_operation text, p_resource_id uuid, p_input jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
      ...
      -- Ids as well as names. Two children in one class can share a display
      -- name, and matching a guardian to a child by name would attach one
      -- family's contact to the other child's record.
      'relatedStudentIds', case when v_staff then coalesce((
        select jsonb_agg(st.id order by st.display_name)
        from public.guardian_links gl
        join public.students st on st.id = gl.student_id and st.deleted_at is null
        where gl.guardian_id = pg.user_id and st.school_id = v_school
          and gl.status = 'verified' and (gl.expires_at is null or gl.expires_at > now())

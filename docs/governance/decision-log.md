...
| DL-030 | 2026-09-13 | Add API-042 for remaining school/product operations and SAFE-043 for communications moderation/safeguarding; strengthen MOB-070 to require independent feature modules, typed presentation data, localization, accessibility and claimed-link handling | Decided for roadmap scope; implementation outstanding | Prevents academic-API completion from being misreported as a launch-ready product and makes store UGC/mobile quality requirements blocking gates | `docs/evidence/launch-readiness/2026-09-13-repository-audit.md`; `instructions.md` §20 |
| DL-031 | 2026-09-13 | Replace the 4,842-line student feature monolith with a 15-line compatibility barrel and 15 independently compiled presentation libraries, each capped below 650 lines by an architecture test; retain and count 32 direct preview-database calls until MOB-070 replaces them | Decided and implemented locally | Removes the file-size and `part`-coupling hotspot without falsely declaring student persistence, typed models, localization, accessibility or synchronization complete | `test/hotspot_architecture_test.dart`; `docs/evidence/launch-readiness/2026-09-13-repository-audit.md` |
| DL-032 | 2026-09-13 | AUTH-030 makes the Hono API the authority for identity: every token is verified against JWKS (signature first, then iss/aud/exp/nbf/iat/sub, asymmetric algorithms only), roles and tenants come from `public.memberships` and never from a claim, revocation uses a per-user not-before watermark so sign-out-everywhere beats token expiry, recent auth is a single-use hashed grant replacing the broken `iat` check, and the API holds EXECUTE on thirteen `private.auth_*` functions with no table, column or sequence grant | Decided for local/disposable scope | Completes the AUTH-030 engineering surface; does not close the Phase 3 gate, which still needs AUTH-031, a threat model and auth penetration test, and provider/store prerequisites | ADR-0016; `docs/evidence/phase-3/README.md` |
| DL-033 | 2026-09-13 | Apple guideline 4.8 posture: implement and ship native Sign in with Apple on Apple platforms. The education/enterprise exception is **not** claimed, because Google and Microsoft sign-in currently create the primary account and claiming it would be untrue. The owned HTTPS callback `https://app.studafy.io/auth/callback` is the primary redirect target; the custom scheme stays as a fallback, narrowed by `AuthCallbackGuard` and backed by PKCE, until domain verification is possible | Decided | Unblocks the 4.8 row of the §23 rejection register at the code level. Physical-device testing (D1), domain verification (D5), and App Review acceptance remain open; revisiting requires a new ADR if signup becomes invitation-only under API-042 | ADR-0016; `docs/release/auth030-site-association.md` |
| DL-034 | 2026-09-13 | Session material moves to the platform keystore: `flutter_secure_storage` holds both the Supabase session and the PKCE code verifier, replacing the SDK default of shared preferences; `sign_in_with_apple` and `crypto` are added for the native Apple flow and its nonce binding. One DB-021 pgTAP assertion is amended from "future runtime roles are inactive" to "runtime roles never inherit or bypass RLS; only the API role is activated", with the new surface asserted separately in `auth030_grants.sql` | Decided | Removes the plaintext refresh-token exposure; records the three new Flutter dependencies and the single deliberate change to DB-021's test expectations | ADR-0016; `supabase/tests/auth030_grants.sql` |
